cd "clubes"
md "serie a"
cd..
md "serie b"
cd..
md "Atlético-MG" "Palmeiras" "Fortaleza" "Bragantino" "Flamengo" "Athletico-PR" "Atlético-GO" "Ceará" "Internacional" "Santos" "Corinthians" "Juventude" "Bahia"
"São Paulo" "Fluminense" "Cuiabá" "Sport" "América-MG" "Grêmio" "Chapecoense"
cd ..
cd "serie b"
md "Coritiba" "Goiás" "Avaí" "CRB" "Sampaio Corrêa" "Guarani" "Náutico" "Botafogo" "Operário-PR" "CSA" "Vasco" "Remo" "Brusque" "Cruzeiro" "Vila Nova" "Ponte Preta"
"Londrina" "Vitória" "Confiança" "Brasil de Pelotas"

